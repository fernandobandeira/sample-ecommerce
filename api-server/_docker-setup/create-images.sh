...
for ((i = 0; i < ${#array[@]}; ++i)); do
  ...
  SERVICE=$(echo ${array[$i]}-service | cut -d'/' -f 2)

  # we delete the image if it exists already
  docker rmi fernandobandeira/$SERVICE
  ...
  docker tag $IMAGE_ID fernandobandeira/$SERVICE:latest

  # we publish our image to our docker hub account
  docker push fernandobandeira/$SERVICE:latest
